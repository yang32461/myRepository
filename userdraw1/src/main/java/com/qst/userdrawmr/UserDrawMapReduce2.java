...
import com.qst.userdraw1.UserDraw;
...
public class UserDrawMapReduce2 {
		...
		public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
			...
			for (Text t : values) {
				...
				if (appID.length() > 0) { // appID不能为空
                    ...
                    long times = Long.parseLong(dataArray[4]);//使用该app的时间
	...
	private static  UserDraw createDrawData(String[] dataArray, //
		...
		// 初始化
		userDraw.initAge(age1, age2, age3, age4, age5, times);
		userDraw.initSex(male, female,times);
